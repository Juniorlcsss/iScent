...
Additional files converted in batch mode:
    - feature_select.h  -> feature_select.h  (fixed naming for Arduino)
    - feature_stats.h   -> feature_stats.h   (full z-score stats, fixed naming)
    - ensemble_weights.h -> ensemble_weights.h (pass-through validation)
"""
...
#===========================================================================================================
#Feature selection loader
#===========================================================================================================
class FeatureSelectionInfo:
    """Loads feature_select.h to determine selected feature count and indices"""

    def __init__(self):
        self.selected_count = None
        self.original_count = FULL_FEATURE_COUNT
        self.selected_indices = []
        self.selected_means = []
        self.selected_stds = []
        self.loaded = False

    def load_from_header(self, path):
        """Parse feature_select.h generated by the trainer"""
        with open(path, 'r') as f:
            content = f.read()

        #SELECTED_FEATURE_COUNT
        m = re.search(r'#define\s+SELECTED_FEATURE_COUNT\s+(\d+)', content)
        if m:
            self.selected_count = int(m.group(1))

        m = re.search(r'#define\s+ORIGINAL_FEATURE_COUNT\s+(\d+)', content)
        if m:
            self.original_count = int(m.group(1))

        #SELECTED_INDICES
        m = re.search(r'SELECTED_INDICES\s*$$.*?$$\s*=\s*\{([^}]+)\}', content)
        if m:
            self.selected_indices = [int(x.strip()) for x in m.group(1).split(',') if x.strip()]

        #SELECTED_MEANS
        m = re.search(r'SELECTED_MEANS\s*$$.*?$$\s*=\s*\{([^}]+)\}', content)
        if m:
            self.selected_means = [float(x.strip()) for x in m.group(1).split(',') if x.strip()]

        #SELECTED_STDS
        m = re.search(r'SELECTED_STDS\s*$$.*?$$\s*=\s*\{([^}]+)\}', content)
        if m:
            self.selected_stds = [float(x.strip()) for x in m.group(1).split(',') if x.strip()]

        #validate
        if self.selected_count and self.selected_indices:
            if len(self.selected_indices) != self.selected_count:
                print(f"WARNING: SELECTED_FEATURE_COUNT ({self.selected_count}) != "
                    f"len(SELECTED_INDICES) ({len(self.selected_indices)})")
                self.selected_count = len(self.selected_indices)

        self.loaded = (self.selected_count is not None and len(self.selected_indices) > 0)
        if self.loaded:
            print(f"Loaded feature selection: {self.selected_count}/{self.original_count} features selected")
        return self.loaded

    def try_load(self, search_dir):
        """Try to find and load feature_select.h from the given directory"""
        p = Path(search_dir) / "feature_select.h"
        if p.exists():
            return self.load_from_header(str(p))
        return False

    def get_feature_name(self, selected_idx):
        """Get the original 73-feature name for a selected feature index"""
        if self.loaded and selected_idx < len(self.selected_indices):
            original_idx = self.selected_indices[selected_idx]
            if original_idx < len(FEATURE_NAMES_73):
                return FEATURE_NAMES_73[original_idx]
            return f"feature_{original_idx}"
        return f"selected_{selected_idx}"

    def get_selected_feature_count(self):
        """Return the number of features models were trained on"""
        if self.loaded and self.selected_count:
...
def get_model_feature_count(binary_feature_count):
    """
    Determine the correct feature count for header generation.
    Uses feature_select.h if available, otherwise falls back to binary value.
    """
    if _feature_selection.loaded:
        expected = _feature_selection.get_selected_feature_count()
        if expected and expected != binary_feature_count:
            print(f"  NOTE: Binary says {binary_feature_count} features, "f"feature_select.h says {expected}. Using {expected}.")
            return expected
    return binary_feature_count


def get_feature_name(idx):
    """Get feature name, using selection info if available"""
    if _feature_selection.loaded:
        return _feature_selection.get_feature_name(idx)
    if idx < len(FEATURE_NAMES_73):
        return FEATURE_NAMES_73[idx]
...
class BaseModelConverter(ABC):
    ...
    def _write_file_header(self, f, model, info):
        ...
        if _feature_selection.loaded:
            f.write(f" (selected from {_feature_selection.original_count})")
        f.write("\n")
        f.write(f"// Classes: {self.num_classes} ({', '.join(CLASS_NAMES)})\n")
        if info:
            f.write(f"// Info: {info}\n")
        f.write("\n")
...
class DTModelConverter(BaseModelConverter):
    ...
    def load_binary(self, filepath):
        ...
        #feature_select.h
        self.feature_count = get_model_feature_count(self.feature_count)
        ...
        with open(output_path, 'w') as f:
            ...
            for i, node in enumerate(self.flat_nodes):
                ...
                if is_leaf:
                    ...
                    fname = get_feature_name(node['feature_index'])
...
class KNNModelConverter(BaseModelConverter):
    ...
    def load_binary(self, filepath):
        with open(filepath, 'rb') as f:
            ...
            bin_feature_count = struct.unpack('<H', f.read(2))[0]
            self.sample_count = struct.unpack('<H', f.read(2))[0]

            self.samples = []
            for _ in range(self.sample_count):
                label = struct.unpack('<B', f.read(1))[0]
                features = list(struct.unpack(f'<{bin_feature_count}f',
                                              f.read(4 * bin_feature_count)))
                self.samples.append({'label': label, 'features': features})

        #use the feature count from the binary for reading
        self.feature_count = get_model_feature_count(bin_feature_count)
        for s in self.samples:
            if len(s['features']) > self.feature_count:
                s['features'] = s['features'][:self.feature_count]
            elif len(s['features']) < self.feature_count:
                s['features'].extend([0.0] * (self.feature_count - len(s['features'])))
    ...
    def generate_header(self, output_path):
        ...
        with open(output_path, 'w') as f:
            self._write_file_header(f, "K-Nearest Neighbors",f"K={self.k}, Samples={self.sample_count}")
...
class RFModelConverter(BaseModelConverter):
    ...
    def load_binary(self, path):
        with open(path, "rb") as f:
            ...
            #feature importance
            self.feature_importance = []
            for _ in range(bin_feature_count):
        ...
        if len(self.feature_importance) > self.feature_count:
            self.feature_importance = self.feature_importance[:self.feature_count]
        elif len(self.feature_importance) < self.feature_count:
            self.feature_importance.extend([0.0] * (self.feature_count - len(self.feature_importance)))
    ...
    def print_summary(self):
        ...
        if self.feature_importance:
            ...
            for idx, imp in sorted_features:
                ...
                bar = "█" * bar_len
                print(f"    [{idx:2d}] {name:25s} {imp:.6f} {bar}")
...
def convert_feature_stats(input_path, output_path):
    """
    Read trainer-generated feature_stats.h and regenerate with naming
    that matches what ml_inference.cpp expects (FEATURE_MEANS/FEATURE_STDS).
    """
    with open(input_path, 'r') as f:
        content = f.read()

    #parse either FEATURE_MEANS or FEATURE_MEAN
    means_match = re.search(r'FEATURE_MEAN[S]?\s*$$.*?$$\s*=\s*\{([^}]+)\}', content)
    stds_match = re.search(r'FEATURE_STD[S]?\s*$$.*?$$\s*=\s*\{([^}]+)\}', content)

    if not means_match or not stds_match:
        raise ValueError("Could not parse feature stats from input file")

    means = [float(x.strip()) for x in means_match.group(1).split(',') if x.strip()]
    stds = [float(x.strip()) for x in stds_match.group(1).split(',') if x.strip()]
    ...
    with open(output_path, 'w') as f:
        ...
        f.write("// NOTE: These are stats for the FULL 73-feature CSV space.\n")
        f.write("//       On Arduino, use SELECTED_MEANS/SELECTED_STDS from feature_select.h\n")
        f.write("//       for the selected feature subset.\n\n")
        ...
        f.write(f"static const float FEATURE_MEANS[{count}] = {{\n")
        for i, m in enumerate(means):
            name = FEATURE_NAMES_73[i] if i < len(FEATURE_NAMES_73) else f"feature_{i}"
            comma = "," if i < count - 1 else ""
            f.write(f"    {m:.8f}f{comma}  // [{i}] {name}\n")
        f.write("};\n\n")

        f.write(f"static const float FEATURE_STDS[{count}] = {{\n")
...
def convert_feature_select(input_path, output_path):
    """
    Read trainer-generated feature_select.h and regenerate with proper
    formatting, comments, and Arduino-compatible naming.
    """
    info = FeatureSelectionInfo()
    info.load_from_header(input_path)

    if not info.load_from_header(input_path):
        raise ValueError("Could not parse feature selection from input file")

    with open(output_path, 'w') as f:
        f.write("// Auto-generated feature selection header\n")
        f.write(f"// Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"// Selected {info.selected_count} features from {info.original_count}\n")
        f.write("// Used by ml_inference.cpp for feature projection and z-score normalization\n\n")

        f.write("#ifndef FEATURE_SELECT_H\n")
        f.write("#define FEATURE_SELECT_H\n\n")

        f.write(f"#define SELECTED_FEATURE_COUNT {info.selected_count}\n")
        f.write(f"#define ORIGINAL_FEATURE_COUNT {info.original_count}\n\n")

        #Selected indices with comments
        f.write(f"static const int SELECTED_INDICES[{info.selected_count}] = {{\n")
        for i, idx in enumerate(info.selected_indices):
            name = FEATURE_NAMES_73[idx] if idx < len(FEATURE_NAMES_73) else f"feature_{idx}"
            comma = "," if i < info.selected_count - 1 else ""
            f.write(f"    {idx}{comma}  // [{i}] {name}\n")
        f.write("};\n\n")

        #Selected means
        if info.selected_means:
            f.write(f"static const float SELECTED_MEANS[{info.selected_count}] = {{\n")
            for i, m in enumerate(info.selected_means):
                orig_idx = info.selected_indices[i] if i < len(info.selected_indices) else -1
                name = FEATURE_NAMES_73[orig_idx] if 0 <= orig_idx < len(FEATURE_NAMES_73) else f"feature_{orig_idx}"
                comma = "," if i < info.selected_count - 1 else ""
                f.write(f"    {m:.8f}f{comma}  // [{i}] {name}\n")
            f.write("};\n\n")

        #Selected stds
        if info.selected_stds:
            f.write(f"static const float SELECTED_STDS[{info.selected_count}] = {{\n")
            for i, s in enumerate(info.selected_stds):
                orig_idx = info.selected_indices[i] if i < len(info.selected_indices) else -1
                name = FEATURE_NAMES_73[orig_idx] if 0 <= orig_idx < len(FEATURE_NAMES_73) else f"feature_{orig_idx}"
                comma = "," if i < info.selected_count - 1 else ""
                f.write(f"    {s:.8f}f{comma}  // [{i}] {name}\n")
    ...
    print(f"Feature select header generated: {output_path}")
    print(f"  Selected: {info.selected_count}/{info.original_count} features")

#===========================================================================================================
#Ensemble Weights Converter
#===========================================================================================================

def convert_ensemble_weights(input_path, output_path):
    """
    Read trainer-generated ensemble_weights.h, validate, and regenerate
    with proper formatting.
    """
    with open(input_path, 'r') as f:
        content = f.read()

    dt_match = re.search(r'DT_WEIGHT\s*=\s*([0-9.e+-]+)f?', content)
    knn_match = re.search(r'KNN_WEIGHT\s*=\s*([0-9.e+-]+)f?', content)
    rf_match = re.search(r'RF_WEIGHT\s*=\s*([0-9.e+-]+)f?', content)

    if not (dt_match and knn_match and rf_match):
        raise ValueError("Could not parse ensemble weights from input file")

    dt_w = float(dt_match.group(1))
    knn_w = float(knn_match.group(1))
    rf_w = float(rf_match.group(1))

    #Compute implied accuracies (weights = accuracy^2)
    dt_acc = dt_w ** 0.5
    knn_acc = knn_w ** 0.5
    rf_acc = rf_w ** 0.5
    ...
    with open(output_path, 'w') as f:
        ...
        f.write(f"// Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"// Weights are accuracy^2 from training evaluation\n")
        ...
        f.write(f"static const float DT_WEIGHT = {dt_w:.6f}f;\n")
        f.write(f"static const float KNN_WEIGHT = {knn_w:.6f}f;\n")
        f.write(f"static const float RF_WEIGHT = {rf_w:.6f}f;\n")

        f.write("\n#endif // ENSEMBLE_WEIGHTS_H\n")

    print(f"Ensemble weights header generated: {output_path}")
...
def convert_all(input_dir=".", output_dir="."):
    """Batch convert all model binary files and auxiliary headers found in a directory"""
    ...
    #try to load feature_select.h
    global _feature_selection
    fs_path = input_dir / "feature_select.h"
    if fs_path.exists():
        try:
            _feature_selection.load_from_header(str(fs_path))
        except Exception as e:
            print(f"WARNING: Could not load feature_select.h: {e}")
    else:
        print(f"NOTE: feature_select.h not found in {input_dir}, " f"will use feature counts from binary files")

    #convert model binaries
    ...
    #convert feature_select.h if present
    if fs_path.exists():
        try:
            print(f"\n{'=' * 60}")
            print(f"Converting: feature_select.h")
            print(f"{'=' * 60}")
            convert_feature_select(str(fs_path), str(output_dir / "feature_select.h"))
            converted += 1
        except Exception as e:
    ...
    if stats_input.exists():
        try:
            print(f"\n{'=' * 60}")
            print(f"Converting: feature_stats.h")
            print(f"{'=' * 60}")
            convert_feature_stats(str(stats_input), str(output_dir / "feature_stats.h"))
            converted += 1
        except Exception as e:
    ...
    #convert ensemble_weights.h if present
    ew_input = input_dir / "ensemble_weights.h"
    if ew_input.exists():
        try:
            print(f"\n{'=' * 60}")
            print(f"Converting: ensemble_weights.h")
            print(f"{'=' * 60}")
            convert_ensemble_weights(str(ew_input), str(output_dir / "ensemble_weights.h"))
            converted += 1
        except Exception as e:
            print(f"  FAILED to convert ensemble_weights.h: {e}")
            failed += 1
    else:
        print(f"\nSkipping ensemble_weights.h (not found in {input_dir})")

    #summary
    total_possible = len(MODEL_FILE_MAP) + 3  #feature_select, feature_stats, ensemble_weights
    ...
    if _feature_selection.loaded:
        print(f"\n  Feature space: {_feature_selection.selected_count} selected " f"from {_feature_selection.original_count}")
...
def main():
    if len(sys.argv) < 2:
        ...
        print()
        print(f"Full feature space: {FULL_FEATURE_COUNT} features")
        print(f"Feature selection: loaded from feature_select.h if present")
        print()
        print("Examples:")
        ...
        print("  python convert_model.py --fix-select feature_select.h feature_select_fixed.h")
        sys.exit(1)
    ...
    if sys.argv[1] == '--fix-select':
        if len(sys.argv) < 3:
            print("Usage: python convert_model.py --fix-select <input.h> [output.h]")
            sys.exit(1)
        input_path = sys.argv[2]
        output_path = sys.argv[3] if len(sys.argv) > 3 else "feature_select.h"
        try:
            convert_feature_select(input_path, output_path)
            print(f"\n>> Feature select fixed: {output_path}")
        except Exception as e:
            print(f"Error: {e}")
            sys.exit(1)
        return

    #fix ensemble weights mode
    if sys.argv[1] == '--fix-ensemble':
        if len(sys.argv) < 3:
            print("Usage: python convert_model.py --fix-ensemble <input.h> [output.h]")
            sys.exit(1)
        input_path = sys.argv[2]
        output_path = sys.argv[3] if len(sys.argv) > 3 else "ensemble_weights.h"
        try:
            convert_ensemble_weights(input_path, output_path)
            print(f"\n>> Ensemble weights fixed: {output_path}")
        except Exception as e:
            print(f"Error: {e}")
            sys.exit(1)
        return
    ...
    #try to load feature_select.h from same directory as input
    global _feature_selection
    input_dir = Path(input_path).parent
    _feature_selection.try_load(str(input_dir))
    ...
    try:
        ...
        if _feature_selection.loaded:
            print(f"Feature selection: {_feature_selection.selected_count} features "f"from {_feature_selection.original_count}")
