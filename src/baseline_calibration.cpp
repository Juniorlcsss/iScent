...
BaselineCalibration::BaselineCalibration():_calibration_state(CALIB_STATE_IDLE),_sample_index(0), _target_samples(0),_valid(false) 
    ...
    memset(_gas1_samples,0,sizeof(_gas1_samples));
    ...
    memset(_gas2_samples, 0, sizeof(_gas2_samples));
...
bool BaselineCalibration::update(const dual_sensor_data_t& sensor_data) {
    ...
    _gas1_samples[_sample_index] = sensor_data.primary.gas_resistances[0];
    ...
    _gas2_samples[_sample_index] = sensor_data.secondary.gas_resistances[0];
    _sample_index++;
    ...
    if (_sample_index >= _target_samples) {
        ...
        _baseline.gas1_baseline=computeMedian(_gas1_samples, _target_samples);
        ...
        _baseline.gas2_baseline=computeMedian(_gas2_samples, _target_samples);
...
float BaselineCalibration::computeMedianGas(const dual_sensor_data_t& data) {
    //TODO:For gas resistance, take median of valid readings, Handle different heater steps if applicable
    // For now, just return the current gas reading (will collect multiple)
    return data.primary.gas_resistances[0];
}
